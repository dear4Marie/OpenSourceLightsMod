...
// ====================================================================================================================================================>
    ...
    // IR SENDER
        IRsend                        irsend;
        //IRsend                        mySender;
        unsigned long                  irSendBufferLong;

        const long IRStateFwd          =     0x20000000;
        const long IRStateRev          =     0x30000000;
        const long IRStateStop         =     0x10000000;
        const long IRStateBrake        =     0x40000000;
        const long IRStateRT           =     0x01000000;
        const long IRStateLT           =     0x02000000;
        const long IRStateCh3Pos1      =     0x00100000;
        const long IRStateCh3Pos2      =     0x00200000;
        const long IRStateCh3Pos3      =     0x00300000;
        const long IRStateCh3Pos4      =     0x00400000;
        const long IRStateCh3Pos5      =     0x00500000;
        const long IRStateCh4Pos1      =     0x00010000;
        const long IRStateCh4Pos2      =     0x00020000;
        const long IRStateCh4Pos3      =     0x00030000;
        const long IRStateBackFire     =     0x00001000;
        const long IRStateOverTaking   =     0x00002000;
        const long IRStateStopDelay    =     0x00003000;
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        ...
        // Channel 4 Winch Mode
        boolean Channel4Present;                                // Channel 4 
        int Channel4Pulse;                                      // 
        int Channel4PulseMin;        
        int Channel4PulseMax;
        int Channel4PulseCenter;
        int Channel4                   =   OFF;                 // State of the Channel 4 switch: On (1), Off (0), Disconnected (-1)
        boolean Channel4Reverse;
        boolean WinchStartFlag         = false;
        int Channel3LastPulse          =   0;
        int Channel4LastCommand        =   OFF;
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        ...
        #define NumStates                    14                 // There are 12 possible states a light can be in: 
        ...
        int LightPin[NumLights] = {10,10,11,6,5,8,15,16};        // These are the Arduino pins to the 8 lights in order from left to right looking down on the top surface of the board. 
                                                                // Note that the six Arduino analog pins can be referred to by numbers 14-19
        int Dimmable[NumLights] = {0,0,0,0,0,0,0,0};            // This indicates which of these pins are capable of ouputting PWM, in order. PWM-capable pins on the Arduino are 3, 5, 6, 9, 10, 11
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        ...
        const byte Channel4_Pin        =     7;                 // The Arduino pin connected to the Channel 4 input
        boolean Failsafe               = true;                  // If we loose contact with the Rx this flag becomes True
    ...
    // DRV8838 MOTOR DRIVER OUTPUT
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        const byte WinchEnablePin     =      3;                 // DRV8838 Enable Pin
        const byte WinchPhasePin      =     12;                 // DRV8838 Phase  Pin
        const byte WinchSleepPin      =     18;                 // DRV8838 Sleep  Pin

    // BOARD OBJECTS
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        const byte GreenLED            =    13;                 // The Arduino pin connected to the on-board Green LED (this is the same as saying pin A4)
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------>
        ...
        struct __eeprom_data {                                  // __eeprom_data is the structure that maps all of the data we are storing in EEPROM
          ...
          int E_Channel4PulseMin;
          int E_Channel4PulseMax;
          int E_Channel4PulseCenter;          
          boolean E_ThrottleChannelReverse;
          boolean E_TurnChannelReverse;
          boolean E_Channel3Reverse;
          boolean E_Channel4Reverse;
          int E_CurrentScheme;
        };
...
//        pinMode(ThrottleChannel_Pin, INPUT);             // Set these pins to input, with internal pullup resistors enabled
...
//        pinMode(Channel4_Pin, INPUT);
        ...
        pinMode(WinchEnablePin, OUTPUT);                        // DRV8838 Enable Pin
        pinMode(WinchPhasePin, OUTPUT);                         // DRV8838 Phase  Pin
        pinMode(WinchSleepPin, OUTPUT);                         // DRV8838 Sleep  Pin
    ...
    // DumpLightSchemeToSerial(0);
    // DumpLightSchemeToSerial(1);
    // DumpLightSchemeToSerial(2);
}
...
// ------------------------------------------------------------------------------------------------------------------------------------------------>    
                    ...
                    {   HoldFlag = false;
                        ...
                        {   if (TurnCommand > 0) 
                            {   CurrentScheme += 1;
                                ...
                                if (CurrentScheme < 0) { CurrentScheme = 1; }
                            }
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------>  
        if ((DEBUG == true) && (DriveModeCommand_Previous != DriveModeCommand))
        {
            ...
            Serial.print(time);
            Serial.print(F("   "));
            Serial.println(irSendBufferLong, HEX);
        }
        ...
        if ((DEBUG == true) && (TurnCommand != 0))
        {
            Serial.print(F("Turn Signal : "));
...
void DumpDebug()
{
    ...
    Serial.print(F("Ch4        "));
    Serial.print(Channel4PulseMin);
    PrintSpaceDash();
    Serial.print(Channel4PulseCenter);
    PrintSpaceDash();
    Serial.println(Channel4PulseMax);
    ...
    Serial.print(F(" - Channel 4        Reverse : "));  
    PrintTrueFalse(Channel4Reverse);
    ...
    if (!Channel4Present) { Serial.print(F("NOT ")); }
    Serial.println(F("CONNECTED")); 

  Serial.print(F("Current Scheme: "));
  Serial.println(CurrentScheme); 
}
