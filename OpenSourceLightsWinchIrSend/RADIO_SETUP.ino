...
unsigned long TotChannel4Pulse = 0;
float TempFloat;
...
// -------------------------------------------------------------------------------------------------------------------------------------------------->        
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------>                  
        ...
        Channel4PulseMin = TypicalPulseCenter;
        Channel4PulseMax = TypicalPulseCenter;
        ...
        do
        {
            ...
            if (ThrottlePulse < ThrottlePulseMin)  { ThrottlePulseMin   = ThrottlePulse; }
            if (ThrottlePulse > ThrottlePulseMax)  { ThrottlePulseMax   = ThrottlePulse; }
            if (TurnPulse     < TurnPulseMin)      { TurnPulseMin       = TurnPulse; }
            if (TurnPulse     > TurnPulseMax)      { TurnPulseMax       = TurnPulse; }
            if (Channel3Pulse < Channel3PulseMin)  { Channel3PulseMin   = Channel3Pulse; }
            if (Channel3Pulse > Channel3PulseMax)  { Channel3PulseMax   = Channel3Pulse; }
            if (Channel4Pulse < Channel4PulseMin)  { Channel4PulseMin   = Channel4Pulse; }
            if (Channel4Pulse > Channel4PulseMax)  { Channel4PulseMax   = Channel4Pulse; }
            // Refresh the timer
            timer.run();
        }
        ...
        if (Channel4PulseMin < MinPossiblePulse)  {Channel4PulseMin = MinPossiblePulse;}
        ...
        if (Channel4PulseMax > MaxPossiblePulse)  {Channel4PulseMax = MaxPossiblePulse;}                
        ...
        eeprom_write(Channel4PulseMin, E_Channel4PulseMin);
        eeprom_write(Channel4PulseMax, E_Channel4PulseMax);
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------>       
        ...
        Serial.println(F("If Channel 4 is a 1-position switch, set it to OFF."));
        Serial.println();
        GreenBlinkSlow(2);
        delay(2000);
        ...
        {        
            ...
            TotChannel4Pulse += pulseIn(Channel4_Pin, HIGH, ServoTimeout);
        ...
        TempFloat = (float)TotChannel4Pulse / (float)Count;
        Channel4PulseCenter = (int)lround(TempFloat);
        ...
        if ((Channel4PulseCenter < MinPossiblePulse) || (Channel4PulseCenter > MaxPossiblePulse))   {Channel4PulseCenter = TypicalPulseCenter; }                
        ...
        eeprom_write(Channel4PulseCenter, E_Channel4PulseCenter);
        ...
        Serial.print(F("Ch4: "));
        Serial.println(Channel4PulseCenter);
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------>                                 
        ...
        {                        
            ...
            Serial.print(F("Channel4 Avg: "));
            Serial.println(TotChannel4Pulse);            
        }
        ...
        if ((TotChannel4Pulse < 1300) && (TotChannel4Pulse > MinPossiblePulse))  { Channel4Reverse = true;        }
       ...
       // Save values to EEPROM
        ...
        eeprom_write(Channel4Reverse, E_Channel4Reverse);     
