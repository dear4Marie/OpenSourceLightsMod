...
{   // If EEPROM has not been used before, we initialize to some sensible, yet conservative, default values.
    ...
    eeprom_write(PulseMin, E_Channel4PulseMin);
    eeprom_write(PulseMax, E_Channel4PulseMax);
    eeprom_write(PulseCenter, E_Channel4PulseCenter);
        
    eeprom_write(false, E_ThrottleChannelReverse);
    eeprom_write(false, E_TurnChannelReverse);
    eeprom_write(false, E_Channel3Reverse);
    eeprom_write(false, E_Channel4Reverse);    
...
{   // If EEPROM has been used before, we run this routine to load all our saved values at startup
    ...
    eeprom_read(Channel4PulseMin, E_Channel4PulseMin);    
    eeprom_read(Channel4PulseMax, E_Channel4PulseMax);    
    eeprom_read(Channel4PulseCenter, E_Channel4PulseCenter);
    ...
    eeprom_read(Channel4Reverse, E_Channel4Reverse); 
